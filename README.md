...
Честное описание того, что ещё предстоит сделать.

**Логи**

Сейчас логирования нет. Метрики есть, но нет никакого структурированного лога с correlation ID, нет контекста операции при ошибке. План: структурированные логи через `logback` в JSON-формате с полями `traceId`, `userId`, `operation`.

**Отправка email**

Нотификация при reset-password не реализована: токен генерируется и сохраняется, но не отправляется. Порт `NotificationSender` выделен в домене; нужно реализовать адаптер через SMTP (простой вариант) или через message broker для децентрализации.

**Кластер и production-деплой**

Helm-чарт написан, но реального K8s-кластера нет — проверка проходила только через `helm template`. CI/CD есть (сборка + тесты), но GitOps-деплоя нет. План: поднять кластер (k3s или managed), добавить Argo CD, вынести DragonflyDB в StatefulSet с PVC.

**Нагрузочное тестирование**

Неизвестно, сколько реальных RPS выдерживает сервис и при каком latency. План: прогонять `k6` (или Gatling) по сценариям login/register/reset, зафиксировать baseline и добавить в CI.

**Persistence без дурабильности**

DragonflyDB работает in-memory без snapshot/persistence. Перезапуск контейнера уничтожает все данные: активные токены, учётные записи пользователей. План: включить RDB/AOF-персистентность в DragonflyDB либо перейти на managed-сервис с гарантированным persistence.

**Алерты**

Grafana есть, дашборды есть, но алертов нет. План: добавить Alertmanager в docker-compose; задать минимальный набор: `error_rate > 1%` за 5 мин, `p99 latency > 500ms`, `dragonfly_up == 0`.
